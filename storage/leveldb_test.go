...
import (
	...
	"testing"
	"time"
	ldb "github.com/jmhodges/levigo"
)
...
func TestC(t *testing.T) {
	if ldb.GetLevelDBMajorVersion() <= 0 {
		t.Errorf("Major version cannot be less than zero")
	}

	dbname := tempDir(t)
	defer deleteDBDirectory(t, dbname)
	env := ldb.NewDefaultEnv()
	cache := ldb.NewLRUCache(1 << 20)

	options := ldb.NewOptions()
	...
	options.SetCompression(ldb.NoCompression)

	roptions := ldb.NewReadOptions()
	roptions.SetVerifyChecksums(true)
	roptions.SetFillCache(false)

	woptions := ldb.NewWriteOptions()
	woptions.SetSync(true)

	_ = ldb.DestroyDatabase(dbname, options)

	db, err := ldb.Open(dbname, options)
	if err == nil {
	...
	err = ldb.RepairDatabase(dbname, options)
	if err != nil {
	...
	// filter
	policy := ldb.NewBloomFilter(10)
	db.Close()
	ldb.DestroyDatabase(dbname, options)
	...
	db.CompactRange(ldb.Range{nil, nil})
...
func CheckGet(t *testing.T, where string, db *ldb.DB, roptions *ldb.ReadOptions, key, expected []byte) {
...
func CheckIter(t *testing.T, it *ldb.Iterator, key, value []byte) {
	if !bytes.Equal(key, it.Key()) {
